import { closeConnection, getDatabase, initializeDatabaseRepositories } from '../../database/utils';
import AppUserRepository, { INVALID_CREDENTIALS_ERROR_MESSAGE } from './repository';
    ...
    describe("when email address does not belong to existing user", () => {
      ...
      describe("when email address belongs to existing user", () => {
        ...
        describe("when password invalid", () => {
          it("throws invalid credentials error", async () => {
            await AppUserRepository.createAppUser(
              "Jean",
              "User",
              emailAddress,
        ...
        describe("when password valid", () => {
          ...
          it("returns user and session", async () => {
            ...
            expect(result).toHaveProperty("appUser");
            expect(result).toHaveProperty("session");
            expect(result.appUser.emailAddress).toEqual(emailAddress);
          });
        });
      });
    });
